...
const AutoGeneratedChannelMarker = 10010
...
func ApplyAutoGeneratedChannelFilter(query *gorm.DB) *gorm.DB {
	return query.Where("priority = ? AND weight = ?", AutoGeneratedChannelMarker, AutoGeneratedChannelMarker)
}

func GetAutoGeneratedChannelGroups() ([]string, error) {
	var groups []string
	err := ApplyAutoGeneratedChannelFilter(DB.Model(&Channel{})).
