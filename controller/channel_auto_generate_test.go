...
func TestAdminOnlyReadsAutoGeneratedChannels(t *testing.T) {
	db := setupAutoChannelControllerTestDB(t)

	normalWeight := uint(100)
	normalPriority := int64(100)
	autoWeight := uint(model.AutoGeneratedChannelMarker)
	autoPriority := int64(model.AutoGeneratedChannelMarker)
	autoBan := 1
...
func TestShouldFilterAutoGeneratedChannelsOnlyForAdminRole(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.want, shouldFilterAutoGeneratedChannels(ctx))
		})
	}
}

func TestGetChannelFiltersAutoGeneratedForAdmin(t *testing.T) {
	db := setupAutoChannelControllerTestDB(t)
...
func TestEnabledListModelsFiltersAutoGeneratedForAdmin(t *testing.T) {
	db := setupAutoChannelControllerTestDB(t)
...
func TestGenerateAutoChannelsWritesRealChannelTable(t *testing.T) {
	db := setupAutoChannelControllerTestDB(t)
	...
	require.NoError(t, model.ApplyAutoGeneratedChannelFilter(db.Model(&model.Channel{})).Find(&generated).Error)
	require.Len(t, generated, 4)
	newGenerated := 0
	for _, channel := range generated {
		require.NotNil(t, channel.Weight)
		require.NotNil(t, channel.Priority)
		assert.Equal(t, uint(model.AutoGeneratedChannelMarker), *channel.Weight)
		assert.Equal(t, int64(model.AutoGeneratedChannelMarker), *channel.Priority)
